...
export default class Logger {
    ...
    async new(param: string | LogState) {
        ...
        if (typeof param == "string") {
            ...
            await this.log("| --- | :-: | :-: |");
            for (const participant of param.participants.sort(
                (a, b) => b.initiative - a.initiative
            )) {
                await this.log(
                    "|",
                    participant.getName().replace("|", "\\|"),
                    "|",
                    participant.initiative.toString(),
                    "|",
                    [
                        ...(participant.status.size
                            ? [...participant.status].map((c) => c.name)
                            : ["-"])
                    ]
                        .join(", ")
                        .replace("|", "\\|"),
            ...
            await this.log("\n### Round 1");
            await this.log(
                `\n##### ${tracker.getOrderedParticipants()[0].getName()}'s turn`
            );
        }
    }
    ...
    logUpdate(messages: UpdateLogMessage[]) {
        ...
        for (const message of messages) {
            ...
            if (message.ac) {
                if (perParticipant.length && !message.status) {
                    perParticipant.push("and");
                } else if (perParticipant.length) {
                    perParticipant.push(",");
                }
            ...
            if (message.status) {
                if (perParticipant.length) {
                    ...
                    perParticipant.push(message.name);
                }
                let status;
                ...
                if (message.saved) {
                    perParticipant.push(`saved against ${status.join(" and ")}`);
                } else {
                    perParticipant.push(`took ${status.join(" and ")} status`);
                }
            }
            if (message.remove_status) {
                ...
                perParticipant.push(`relieved of ${status.join(" and ")} status`);
            }
            toLog.push(perParticipant.join(" "));
        }
        this.log(`${toLog.join(". ")}.`);
    }
}
